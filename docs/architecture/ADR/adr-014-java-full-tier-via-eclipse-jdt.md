...
| -------------- | ---------------------------------------------------------------------- |
| **Status**     | Accepted                                                               |
| **Date**       | 2026-04-20 (proposed), 2026-06-21 (accepted + revised twice), 2026-09-11 (revised: dual-JAR for Java 11 support) |
| **Deciders**   | Alper Hankendi                                                         |
| **Decision**   | Ship `@ctxo/lang-java` as a **single deliverable** combining a tree-sitter syntax tier and standalone Java uber-JARs (`ctxo-jdt-analyzer-11.jar` / `ctxo-jdt-analyzer-17.jar`) that use Eclipse JDT Core (ASTParser + `resolveBindings`) to emit semantic symbols and edges as JSON. The analyzer ships as **two prebuilt JARs** (dual-JAR strategy, JRE 11+ minimum — JDT/ECJ analyzes source levels Java 8→21 independent of the running JRE) **distributed as a separate opt-in npm package `@ctxo/lang-java-analyzer`** (both JARs inside the tarball; integrity + provenance come from npm); the plugin selects the correct JAR via `toolchain-detect.ts` based on the detected JRE major version. Tree-sitter is the always-present baseline and the fallback when JRE 11+ is absent or the analyzer package is not installed. Java analog of ADR-007 (Roslyn) and ADR-013 (Go). |
...
> **Revision (2026-06-21, party-mode architecture review):** A facilitated multi-agent debate (Duke / Aydın / Theo / Demir, refereed by evidence research) revised two earlier decisions: (1) the **two-runtime-tier JDT JAR (jdt17/jdt21) is dropped in favour of a single JRE-17-base JAR** — a single JDT build on JRE 17 analyzes Java 8→21 because source level is independent of the running JRE, so the second tier added complexity with no coverage gain; (2) build-from-source on the user's machine (Maven/`mvnw`) was rejected in favour of a prebuilt JAR. JRE 21 as a future second tier is deferred for re-evaluation, not adopted.

> **Revision 3 (2026-09-11, Java 11 support — dual-JAR strategy):** The original single-JAR decision (JRE 17 base, JDT 3.39.0) is revised to support JRE 11+ via a dual-JAR approach. `toolchain-detect.ts` selects the JAR variant at runtime: JRE 11–16 → `ctxo-jdt-analyzer-11.jar` (JDT 3.33.0, `Bundle-RequiredExecutionEnvironment: JavaSE-11`, analyzes Java 8–19 source); JRE 17+ → `ctxo-jdt-analyzer-17.jar` (JDT 3.39.0, analyzes Java 8–21 source). The JDT version boundary is confirmed: JDT ≤ 3.33.x requires JavaSE-11, JDT ≥ 3.34.0 requires JavaSE-17. Both JARs ship inside the `@ctxo/lang-java-analyzer` npm tarball; `jar-resolve.ts` selects by `jarVariant` and gracefully falls back to the other variant if the exact one is missing. `pom.xml` defines two Maven profiles (`java11`, `java17`); CI must build both. The `maven-compiler-plugin` is pinned to 3.11.0 to ensure `--release` flag support across Maven versions. Minimum JRE gate: `MIN_MAJOR = 11` in `toolchain-detect.ts`.

> **Revision 2 (2026-06-21, distribution brainstorm — supersedes the "GitHub Releases download + SHA-256" decision):** Deep prior-art research (esbuild/sharp/rollup/biome migrations away from postinstall-download; pnpm/Yarn/Bun blocking install scripts by default; GitHub's 60 req/hr unauthenticated rate limit; npm's free SRI integrity + SLSA provenance) plus an end-user-practicality review changed the distribution channel: the JAR is shipped as a **separate opt-in npm package `@ctxo/lang-java-analyzer`** (JAR inside the tarball), NOT lazily downloaded from GitHub Releases. Consequences: **(a)** the manual SHA-256 pin is dropped — npm's lockfile `integrity` + provenance (Ctxo already publishes with `--provenance`) cover it for free; **(b)** acquisition is via `ctxo install java --full-tier`, a **smart default**: `ctxo install java` detects a JRE 17+ and installs the analyzer package when present (full tier), else installs syntax-only with a clear message (`--syntax-only` / `--full-tier` override); **(c)** the active tier is surfaced transparently on every relevant surface (`ctxo index`, `ctxo status`, `ctxo doctor`, MCP `_meta`) so a degraded fallback is never silent; **(d)** the JAR is built in CI **only at release time** (`setup-java` + `mvn package` in `release.yml`), never committed as a binary, and the day-to-day PR pipeline stays Node-only. The `Distribution` and rollout sections below are superseded by this; treat their "GitHub Releases / SHA-256 / jar-download" wording as historical.
...
| Minimum runtime                        | JRE 11 (dual-JAR: JDT 3.33.0 for JRE 11–16, JDT 3.39.0 for JRE 17+; both analyze Java 8→19/21) | Any JDK matching the target source level |
...
### Dual-JAR strategy — JRE 11+ minimum (revised 2026-09-11)
...
The JDT version boundary is confirmed empirically via `Bundle-RequiredExecutionEnvironment` in each JAR's `META-INF/MANIFEST.MF`: JDT ≤ 3.33.x declares `JavaSE-11`; JDT ≥ 3.34.0 declares `JavaSE-17`. Two prebuilt JARs ship inside `@ctxo/lang-java-analyzer`; `toolchain-detect.ts` selects the correct one at runtime based on the detected JRE major version:

| Detected Java major | JAR selected                    | JDT version | Max source level | Tier delivered     |
| ------------------- | --------------------------------| ----------- | ---------------- | ------------------ |
| ≥ 17                | `ctxo-jdt-analyzer-17.jar`      | 3.39.0      | Java 21          | **full**           |
| 11–16               | `ctxo-jdt-analyzer-11.jar`      | 3.33.0      | Java 19          | **full**           |
| < 11 or no JRE      | — (no JAR)                      | —           | —                | tree-sitter syntax |

- `toolchain-detect.ts` parses `java -version` to a major number; ≥ 11 enables the full-tier path, below 11 stays on tree-sitter. `jarVariantFor(major)` returns `'java17'` for major ≥ 17, `'java11'` otherwise.
- `jar-resolve.ts` receives the `jarVariant` and resolves the matching JAR from `node_modules/@ctxo/lang-java-analyzer/jar/`. If the exact variant is missing it gracefully falls back to the other.
- **JRE 21 as a future third tier is deferred for re-evaluation**, not adopted.
...
- **Java runtime required for full tier.** A JRE ≥ 11 on `PATH` enables the full-tier path; below 11 or absent → tree-sitter (the always-present baseline). The gate is keyed on the *runtime* major version, never on the analyzed code's source level. `ctxo doctor` surfaces the detected runtime and an install hint when below 11. (JRE 21 as a future third tier is deferred for re-evaluation.)
- **We maintain two Java uber-JARs** (~1500 LOC shared, two Maven profiles) — `maven-compiler-plugin 3.11.0` pinned for `--release` flag support. JDT Core API is stable across minor versions; the `java11` profile pins JDT 3.33.0, the `java17` profile pins JDT 3.39.0.
...
| 2 | JAR build + distribution strategy                   | **Two prebuilt JARs inside the npm tarball** (see Revision 3, 2026-09-11). CI builds both `ctxo-jdt-analyzer-11.jar` (Maven profile `java11`, JDT 3.33.0) and `ctxo-jdt-analyzer-17.jar` (profile `java17`, JDT 3.39.0) and packages them in `@ctxo/lang-java-analyzer`. Runtime selection via `toolchain-detect.ts`. No postinstall hook. SLSA/cosign deferred. |
...
| 7 | Java source level support                           | JRE 11–16 (JDT 3.33.0) analyzes Java 8–19; JRE 17+ (JDT 3.39.0) analyzes Java 8–21. Source level is independent of the running JRE. Default: auto-detect from `pom.xml` `<maven.compiler.source>` or Gradle `sourceCompatibility`; fall back to the highest level the build supports. |
| 8 | Runtime tier & minimum JRE                           | **Dual-JAR, JRE-11 minimum** (see *Dual-JAR strategy*, Revision 3). JRE ≥ 11 → full-tier JAR selected by variant; < 11 / absent → tree-sitter. `toolchain-detect.ts` parses `java -version` to a major number, `jarVariantFor()` maps to `java11`/`java17`. JRE-21 third tier deferred for future re-evaluation. |
...
7. **Toolchain detection:** `toolchain-detect.ts` parses `java -version` → major number → ≥ 11 enables full tier, below 11 stays on tree-sitter; `jarVariantFor(major)` selects `java17` (≥ 17) or `java11` (11–16); `ctxo doctor` reports the detected runtime and active JAR variant.
...
9. **CI regression test:** fixture Maven project where tree-sitter emits 0 `calls` edges but JDT must emit ≥ 50. Floor check prevents silent degradation. Run the JDT path on both a JRE 11 leg (java11 profile, supported floor) and a JRE 17 leg (java17 profile); confirm each resolves its respective source fixture.
