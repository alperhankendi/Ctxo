...
in `package.json` exposes the `ctxo` binary (via `npm install -g @ctxo/cli` or `npx -y @ctxo/cli`).
...
- **Package**: single npm package `@ctxo/cli`, `bin` field exposes the `ctxo` binary (install globally via `npm install -g @ctxo/cli`)
