...
**Commit:** `fa4701d` — Add MCP server with get_logic_slice tool and privacy masking pipeline
...
| 3 | deleteFileData only removes from_symbol edges (was deleting cross-file edges) | Critical |
| 4 | Cache SymbolGraph in get_logic_slice (was full DB scan per MCP call) | High |
...
**Commit:** `d8f2f5c` — Restore to_symbol FK with graceful error handling (tryInsertEdge + two-phase bulkWrite)
...
| # | Fix | Root Cause |
|---|-----|------------|
| 1 | AWS_SECRET false positive on git hashes | Regex matched any 40-char alphanumeric; fixed to require mixed case |
