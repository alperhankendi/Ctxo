...
describe('CSharpAdapter — symbol extraction', () => {
  ...
  it('extracts public class with namespace qualification', () => {
  ...
  it('extracts public interface with namespace', () => {
    const source = readFixture('csharp-sample.cs.fixture');
    const symbols = adapter.extractSymbols('Services/Payment.cs', source);
  ...
  it('extracts public methods inside classes', () => {
    const source = readFixture('csharp-sample.cs.fixture');
    const symbols = adapter.extractSymbols('Services/Payment.cs', source);
  ...
  it('extracts constructor as method', () => {
    const source = readFixture('csharp-sample.cs.fixture');
    const symbols = adapter.extractSymbols('Services/Payment.cs', source);
  ...
  it('extracts enum as type kind', () => {
    const source = `
namespace App {
    public enum Status { Active, Inactive }
}
`;
    const symbols = adapter.extractSymbols('src/Status.cs', source);
...
`;
    const symbols = adapter.extractSymbols('src/Point.cs', source);
...
describe('CSharpAdapter — edge extraction', () => {
  ...
  it('extracts using directives as import edges', () => {
    const source = readFixture('csharp-sample.cs.fixture');
    const edges = adapter.extractEdges('Services/Payment.cs', source);

    const importEdges = edges.filter(e => e.kind === 'imports');
    expect(importEdges.some(e => e.to.includes('System'))).toBe(true);
  });

  it('extracts implements edge for class implementing interface', () => {
    const source = readFixture('csharp-sample.cs.fixture');
    const edges = adapter.extractEdges('Services/Payment.cs', source);
  ...
  it('extracts extends edge for class inheritance', () => {
    const source = `
namespace App {
    public class Base { }
...
`;
    const edges = adapter.extractEdges('src/Child.cs', source);
    const extendsEdge = edges.find(e => e.kind === 'extends');
    expect(extendsEdge).toBeDefined();
    expect(extendsEdge!.to).toContain('Base');
  });

  it('returns empty edges for file with no public symbols', () => {
    const source = `
namespace App {
    internal class Secret { }
}
`;
    const edges = adapter.extractEdges('src/Secret.cs', source);
...
describe('CSharpAdapter — complexity extraction', () => {
  ...
  it('counts if statement as branch', () => {
    const source = readFixture('csharp-sample.cs.fixture');
    const metrics = adapter.extractComplexity('Services/Payment.cs', source);
  ...
  it('returns complexity 1 for simple method', () => {
    const source = `
namespace App {
    public class Svc {
...
`;
    const metrics = adapter.extractComplexity('src/Svc.cs', source);
...
describe('CSharpAdapter — edge cases', () => {
  it('resolves base type via symbol registry', () => {
    const adapter = new CSharpAdapter();
...
`;
    const edges = adapter.extractEdges('Services/Worker.cs', source);
    const implEdge = edges.find(e => e.kind === 'implements');
    expect(implEdge).toBeDefined();
  ...
  it('resolves base type via registry with unqualified name match', () => {
    const adapter = new CSharpAdapter();
    const registry = new Map<string, import('../../../core/types.js').SymbolKind>();
...
`;
    const edges = adapter.extractEdges('App/MyService.cs', source);
    const extendsEdge = edges.find(e => e.kind === 'extends');
    expect(extendsEdge).toBeDefined();
...
`;
    const symbols = adapter.extractSymbols('src/UserDto.cs', source);
    const rec = symbols.find(s => s.name === 'App.UserDto');
    expect(rec).toBeDefined();
    expect(rec!.kind).toBe('class');
  });

  it('extracts symbols without namespace', () => {
    const adapter = new CSharpAdapter();
    const source = `
...
`;
    const symbols = adapter.extractSymbols('src/Global.cs', source);
  ...
  it('handles multiple using directives', () => {
    const adapter = new CSharpAdapter();
    const source = `
using System;
using System.Linq;
...
`;
    const edges = adapter.extractEdges('src/Svc.cs', source);
    const imports = edges.filter(e => e.kind === 'imports');
    expect(imports).toHaveLength(3);
  });

  it('complexity counts foreach and while loops', () => {
    const adapter = new CSharpAdapter();
    const source = `
namespace App {
...
`;
    const metrics = adapter.extractComplexity('src/Processor.cs', source);
    expect(metrics).toHaveLength(1);
