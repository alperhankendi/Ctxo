...
describe('TsMorphAdapter — error paths', () => {
  ...
  it('returns empty array and logs stderr for syntax error file', async () => {
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const source = 'export function broken( { return }';
    const symbols = await adapter.extractSymbols('src/broken.ts', source);
  ...
  it('returns empty edges for syntax error file', async () => {
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const source = 'export class { invalid }';
    const edges = await adapter.extractEdges('src/broken.ts', source);

    expect(edges).toEqual([]);
    spy.mockRestore();
  });

  it('returns empty complexity for syntax error file', async () => {
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
    const source = 'function {{{ broken';
    const metrics = await adapter.extractComplexity('src/broken.ts', source);
...
describe('TsMorphAdapter — import edge detection', () => {
  ...
  it('detects "imports" edge for named import from relative path', async () => {
    const source = `
      import { Foo } from './foo';
      export function bar() { return new Foo(); }
    `;
    const edges = await adapter.extractEdges('src/bar.ts', source);
  ...
  it('returns empty edges for file importing only from node_modules', async () => {
    const source = `
      import { z } from 'zod';
      export const schema = z.string();
    `;
    const edges = await adapter.extractEdges('src/schema.ts', source);
...
describe('TsMorphAdapter — preloading lifecycle', () => {
  it('extractEdges without preloading still works (backward compat)', async () => {
    const adapter = new TsMorphAdapter();
    const source = `
    ...
    const edges1 = await adapter.extractEdges('src/bar.ts', source);
    const edges2 = await adapter.extractEdges('src/bar.ts', source);
  ...
  it('extractSymbols and extractComplexity still clean up when preloaded', async () => {
    const adapter = new TsMorphAdapter();
    ...
    const symbols = await adapter.extractSymbols('src/b.ts', 'export function b(): void {}');
    expect(symbols).toHaveLength(1);
    expect(symbols[0]!.name).toBe('b');

    // extractComplexity should still work
    const metrics = await adapter.extractComplexity('src/c.ts', 'export function c(): void { if (true) {} }');
    expect(metrics).toHaveLength(1);

    adapter.clearProjectSources();
  });

  it('clearProjectSources restores single-file cleanup behavior', async () => {
    const adapter = new TsMorphAdapter();
    ...
    `;
    const edges = await adapter.extractEdges('src/b.ts', source);
  ...
  it('malformed source in preloaded map does not crash other files', async () => {
    const adapter = new TsMorphAdapter();
...
describe('TsMorphAdapter — complexity edge cases', () => {
  ...
  it('counts logical operators as complexity branches', async () => {
    const source = `export function check(a: boolean, b: boolean): boolean {
      return a && b || !a;
    }`;
    const metrics = await adapter.extractComplexity('src/check.ts', source);

    expect(metrics).toHaveLength(1);
    expect(metrics[0]!.cyclomatic).toBeGreaterThan(1);
  });

  it('handles function with for/while/do loops', async () => {
    const source = `export function loops(): void {
      for (let i = 0; i < 10; i++) {}
      while (true) { break; }
      do {} while (false);
    }`;
    const metrics = await adapter.extractComplexity('src/loops.ts', source);
